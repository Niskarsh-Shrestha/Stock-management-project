...
$forgot_code = str_pad((string)random_int(0, 9999), 4, '0', STR_PAD_LEFT);
$upd = $conn->prepare("UPDATE users SET forgot_code = ? WHERE id = ?");
$upd->bind_param("si", $forgot_code, $row['id']);
$upd->execute();

// Send forgot code email using Resend API
